...
const MetricCard: React.FC<MetricCardProps> = ({
  ...
  // Use real data if available, otherwise generate demo data for display
  const displayChartData = React.useMemo(() => {
    ...
    const baseValue = typeof value === 'number' ? value / 7 : 1000;
    return Array.from({ length: 7 }, (_, i) => ({
      name: `${22 + i}/07`,
      value: Math.floor(baseValue + (Math.random() * baseValue * 0.4) - (baseValue * 0.2))
    }));
  }, [chartData, value]);
  ...
  const renderChart = () => {
    ...
    return (
      <ResponsiveContainer width="100%" height="100%">
        <ChartComponent data={dataToUse} {...chartConfig}>
          <defs>
  ...
  return (
    ...
    >
      ...
      <div className={styles.cardDateRange}>
        <span>{displayChartData[0]?.name}</span>
        <span>{displayChartData[displayChartData.length - 1]?.name}</span>
      </div>
    </div>
  );
};
